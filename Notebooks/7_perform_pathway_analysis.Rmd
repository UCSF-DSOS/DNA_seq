...
```{r}
brca <- read.delim(file="tcga_brca_mut_top100.tsv", stringsAsFactors=FALSE)
head(brca)
```
...
```{r}
png("./Results/barplot_brca.png", width=960)
...
```{r message=FALSE}
png("../Results/dotplot_brca.png", width=960)
...
```{r}
png("../Results/emap_brca.png", width=960)
emapplot(brca_pwy, showCategory = 10)
dev.off()
```
